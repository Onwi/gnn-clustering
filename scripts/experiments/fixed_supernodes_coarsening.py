...
from pooling_genomic.datasets import get_genomic_classification_dataset, get_tcga_cohort_classification_datasets, PCRunIndicesLoader
from pooling_genomic.models import build_fixed_supernodes_coarsening_model
...
def train_and_validate_model(
    ...
    model = build_fixed_supernodes_coarsening_model(
...
def train_and_test_model(results, args, path_experiment, first_level, random_state, 
                         indices_loader: PCRunIndicesLoader | None = None):
    max_levels = args.max_n_levels
    path_dataset = Path(args.path_dataset)
    path_levels = Path(args.path_levels)
...
def test_tuned_model(results, first_level, args, path_experiment, random_state, indices_loader: PCRunIndicesLoader | None = None):
    ...
    for first_level in range(0, args.max_n_levels+1):
        for weighted_pooling in [False, True]:
            ...
            path_experiment = (
                ...
                / f"firstlevel{first_level}_rep{rep}_wpool{weighted_pooling}_nlevels{args.max_n_levels}"
            )
