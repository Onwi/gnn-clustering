...
def HEM(W, levels, rid=None, rng=np.random.default_rng(123)):
    """
...
def get_nodes_in_cluster(parents, level):
    """Get nodes present in the clusters at the given level."""
    
    cluster_indices = np.arange(0, len(parents[level]))
    clusters = {}
